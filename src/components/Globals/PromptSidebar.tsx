...
import ScrapeSingleURL from "./ScrapeSingleURL";
...
const PromptSidebar: React.FC<PromptSidebarProps> = ({
  ...
  const handleWriteArticle = async () => {
    setIsLoading(true);
    setCountdown(0); // Start the countdown from 0
    try {
      let apiEndpoint = "";
      let requestBody = {
        keywords,
    ...
    <Drawer
      title={
        <div className="flex items-center justify-between">
          <span>Create a new page with AI</span>
          {countdown !== null && (
            <span className="ml-4 mr-2 rounded bg-blue-100 px-2 py-1 text-sm font-medium text-blue-800">
      ...
      footer={
        <button
          ...
          disabled={isLoading}
          className="w-full rounded-md bg-blue-600 px-4 py-3 font-bold text-white shadow-md transition-colors hover:bg-blue-700 disabled:bg-blue-300"
        >
          {isLoading ? "Generating Article..." : "Write Article"}
        </button>
      }
      extra={
        <Space>
      ...
      <Form layout="vertical" hideRequiredMark className="font-satoshi">
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              className="w-full rounded-md border border-slate-200 p-3 text-xl font-semibold focus:border-blue-500 focus:outline-none focus:ring-blue-500"
              placeholder="Example: Dr. Raunak Jahan"
              value={keywords}
          ...
          <div className="space-y-2">
            ...
            <textarea
              className="w-full rounded-md border border-slate-200 p-3 text-base font-medium focus:border-blue-500 focus:outline-none focus:ring-blue-500"
              placeholder="Example: Dr raunak jahan, Gynecologist & Obstetrician, Chittagong Women Doctor, etc."
              value={lsiKeywords}
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-slate-800">
              Write your relevant information about the topic here so that the
              bot can respond better. You can edit the scraped data or add your
              own information.
            </p>
            ...
            <ScrapeSingleURL
              ...
              onScrape={handleImageUpload}
              updateFormDataFromAI={updateFormData}
            />

            {/* Relevant Information Textarea */}
            <div className="mt-4">
              <textarea
                className="h-80 w-full rounded-md border border-slate-200 p-3 text-base focus:border-blue-500 focus:outline-none focus:ring-blue-500"
                value={relevantInformations}
                onChange={(e) => setRelevantInformations(e.target.value)}
                placeholder="Enter or edit relevant information here..."
              />
            </div>
          </div>
